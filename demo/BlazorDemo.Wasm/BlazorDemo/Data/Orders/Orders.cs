...
namespace BlazorDemo.Data {
    public class OrderRepository {
        ...
        public Task<IEnumerable<Order>> GetData() {
            orders = GenerateData();
            ...
            foreach(var field in newValue.Keys)
                switch(field) {
                    ...
                    case nameof(order.OrderDate):
                        order.OrderDate = (DateTime)newValue[field];
                        break;
                    case nameof(order.UnitPrice):
                        order.UnitPrice = (decimal)newValue[field];
                        break;
                    case nameof(order.UnitsInOrder):
                        order.UnitsInOrder = (int)newValue[field];
                        break;
                    case nameof(order.OrderStatus):
                        order.OrderStatus = (OrderStatus)newValue[field];
                        break;
                }
        }
        public void Add(IDictionary<string, object> newValue) {
            Order order = new Order();
            Update(order, newValue);
